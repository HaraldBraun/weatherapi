...
Die Forecast‑API arbeitet ausschließlich koordinatenbasiert (lat/lon). [api.openmeteo.com]  
Zur Vorverarbeitung dient die Geocoding‑API: Sie nimmt Ortsnamen oder Postleitzahlen und liefert dazu passende Orte zurück.
 [digitalvalley.de]
...
1. User tippt in ein Textfeld (z. B. „Nord“)
2. UI schickt an Geocoding‑Actor:
   ```
   GetSuggestions("Nord", "DE")
   ```
3. Geocoding‑Actor ruft API auf:
   ```Plain Text
   https://geocoding-api.open-meteo.com/v1/search?name=Nord&countryCode=DE&count=10&language=de  
   ```
4. API liefert Liste von möglichen Orten
5. UI‑Actor füllt die Suggestion‑Listbox / Dropdown
6. User klickt auf einen Eintrag
7. UI extrahiert latitude + longitude
8. UI sendet Abfrage an Weather‑Actor → Forecast‑API

## 5. Forecast‑Abfrage mit Koordinaten
Open‑Meteo Forecast API nutzt ausschließlich Koordinaten. [api.openmeteo.com]  
...
Basierend auf der API‑Struktur: [digitalvalley.de]
...
1. Ortsnamensuche mit
   - name=
   - countryCode=DE
   - language=de
2. Auswahlliste anzeigen
3. Koordinaten übernehmen
4. Forecast‑API aufrufen
